...
describe("loadReconciliation (injected fetcher, no network)", () => {
  it("reconciles registrations vs payments into the three buckets", async () => {
    ...
    expect(result.registeredPaid.map((x) => x.registration.fullName)).toEqual([
      "Kofi",
    ]);
...
describe("loadOptOuts (injected fetcher, no network)", () => {
  it("returns the opted-out phones as a set, dropping null rows", async () => {
    ...
    expect(await loadOptOuts(fetcher)).toEqual(
      new Set(["+233244000001", "+233209999999"]),
    );
  });
...
describe("sent_messages audit trail", () => {
  ...
  it("recordSentMessages inserts via the injected inserter", async () => {
    ...
    expect(
      await recordSentMessages(toSentMessageRows(messages, outcomes), inserter),
    ).toBe(true);
    expect(inserter).toHaveBeenCalledWith(
      "sent_messages",
      expect.arrayContaining([
        expect.objectContaining({ partner_ref: "reg_0" }),
      ]),
    );
  });
  ...
  it("loads accepted message identities for batch-send deduplication", async () => {
    const fetcher = vi.fn(async () => [
      { to_phone: "+233244000001", body: "Hi Ama, thank you." },
      { to_phone: null, body: "ignored" },
      { to_phone: "+233244000002", body: null },
    ]) as Fetcher;

    await expect(
      loadAcceptedSentMessageKeys("thank_you", fetcher),
    ).resolves.toEqual(
      new Set([sentMessageKey("+233244000001", "Hi Ama, thank you.")]),
    );
    expect(fetcher).toHaveBeenCalledWith(
      "sent_messages?select=to_phone,body&kind=eq.thank_you&status=in.(queued,sent)&limit=5000",
    );
  });
});

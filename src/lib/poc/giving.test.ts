...
const branchMap = new Map([
  ...
  [
    "+233240000002",
    { branch: "Atonsu", name: "Kofi Mensah", country: "Ghana" },
  ],
]);
...
const payments: DbGivingPayment[] = [
  {
    ...
    payer_phone_e164: "+233240000001",
    amount_minor: 10_000,
    currency: "GHS",
    paid_at: "2026-06-01T03:00:00+00:00",
  },
  {
    reference: "r2",
    payer_name: "Kofi Mensah",
    payer_phone_e164: "+233240000002",
    amount_minor: 5_000,
    currency: "GHS",
    paid_at: "2026-06-15T09:20:00+00:00",
  },
  {
    reference: "r3",
    payer_name: "Stranger",
    payer_phone_e164: "+233249999999",
    amount_minor: 2_500,
    currency: "GHS",
    paid_at: "2026-07-02T11:00:00+00:00",
  },
];
...
describe("filterGiving", () => {
  ...
  it("filters by branch, including the unattributed bucket", () => {
    expect(
      filterGiving(entries, { branch: "Qodesh" }).map((e) => e.reference),
    ).toEqual(["r1"]);
    expect(
      filterGiving(entries, { branch: UNATTRIBUTED }).map((e) => e.reference),
    ).toEqual(["r3"]);
  });

  it("filters by name case-insensitively on a substring", () => {
    expect(
      filterGiving(entries, { name: "kofi" }).map((e) => e.reference),
    ).toEqual(["r2"]);
  });
...
describe("summarizeGiving", () => {
  it("totals the filtered set, not the whole ledger", () => {
    ...
    const filtered = summarizeGiving(
      filterGiving(entries, { branch: "Qodesh" }),
    );
  ...
  it("breaks down by branch, largest first", () => {
    expect(summarizeGiving(entries).byBranch[0]).toEqual({
      branch: "Qodesh",
      amountMinor: 10_000,
      count: 1,
    });
  ...
  it("branch totals always reconcile to the overall total", () => {
    ...
    expect(s.byBranch.reduce((n, b) => n + b.amountMinor, 0)).toBe(
      s.totalMinor,
    );
  });
});
...
describe("sortByDateDesc / branchOptions", () => {
  it("puts the newest gift first", () => {
    expect(sortByDateDesc(entries).map((e) => e.reference)).toEqual([
      "r3",
      "r2",
      "r1",
    ]);
...
describe("statement rows (Decision 0008 §6)", () => {
  const withNoise = toEntries(
    [
      ...
      {
        reference: "n1",
        payer_name: "Ecobank MobileApp",
        payer_phone_e164: null,
        amount_minor: 10_000,
        currency: "GHS",
        paid_at: "2026-07-03T10:00:00+00:00",
      },
      {
        reference: "n2",
        payer_name: "INTEROPERABILITY PULL OVA",
        payer_phone_e164: null,
        amount_minor: 5_000,
        currency: "GHS",
        paid_at: "2026-07-04T10:00:00+00:00",
      },
    ],
    branchMap,
  );
  ...
  it("flags bank/interop artifacts as not-a-person", () => {
    ...
    expect(withNoise.find((e) => e.reference === "r1")?.isStatement).toBe(
      false,
    );
  });

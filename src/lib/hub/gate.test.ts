...
describe("decideHubRoute", () => {
  it("without a hub session: hub pages go to login, hub APIs 401, other paths fall through to staff rules", () => {
    expect(decideHubRoute("/hub", null)).toEqual({
      kind: "redirect",
      to: "/login",
    });
    expect(decideHubRoute("/hub/password", null)).toEqual({
      kind: "redirect",
      to: "/login",
    });
    expect(decideHubRoute("/api/hub/password", null)).toEqual({
      kind: "unauthorized",
    });
  ...
  it("a hub session is corralled into the hub area — it never reaches /poc or staff APIs", () => {
    ...
    expect(decideHubRoute("/api/poc/send", session(false))).toEqual({
      kind: "redirect",
      to: "/hub",
    });
  ...
  it("must-change sessions are forced to the password screen and can reach nothing else in the hub area", () => {
    expect(decideHubRoute("/hub", session(true))).toEqual({
      kind: "redirect",
      to: "/hub/password",
    });
    expect(decideHubRoute("/hub/password", session(true))).toEqual({
      kind: "next",
    });
    expect(decideHubRoute("/api/hub/password", session(true))).toEqual({
      kind: "next",
    });
    expect(decideHubRoute("/api/hub/logout", session(true))).toEqual({
      kind: "next",
    });
    expect(decideHubRoute("/api/hub/ingest", session(true))).toEqual({
      kind: "unauthorized",
    });
  });

...
7. **Sending is batched, and staff drive it from the composer.** Choosing the legacy audience reveals a batch selector — 6 batches (5×2,000 + 1,346) with per-batch progress, defaulting to the first with anyone left. Preview and send both act on one batch. The chunking lives in `src/lib/poc/legacy-batches.ts` and is shared with the CLI script, so a batch number means the same people in both. Contacts already sent to keep their slot rather than being filtered out before chunking — dropping them would reslide every later boundary and let staff send the same people twice under two different batch names.
...
1. **Matching is by phone**, MoMo or WhatsApp, against partners of the uploading hub. A match updates `full_name`, both numbers, `church`, `church_id` and `source`.
...
4. **Ambiguity is refused, not guessed.** If a row's two numbers match two *different* existing partners, the row is flagged for the admin to resolve — updating either would silently corrupt the other.
